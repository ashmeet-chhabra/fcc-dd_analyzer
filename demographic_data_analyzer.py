...
def calculate_demographic_data(print_data=True):
    ...
    num_min_workers = df.loc[df['hours-per-week'] == min_hours]

    rich_percentage = 100 * num_min_workers.loc[num_min_workers['salary'] == '>50K'].shape[0] / num_min_workers.shape[0]

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = df.loc[df['salary'] == '>50K', ['native-country']].value_counts().index[0][0]
    highest_earning_country_percentage = 100 * df.loc[df['salary'] == '>50K', ['native-country']].value_counts()[0] / (df.loc[df['native-country'] == highest_earning_country, ['native-country']].shape[0])
